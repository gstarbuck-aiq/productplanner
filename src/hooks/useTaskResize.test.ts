...
describe("useTaskResize", () => {
  ...
  describe("week view — right handle", () => {
    it("should increase duration when dragging right by one week", () => {
      ...
      act(() => {
        ...
        result.current.handleResize(
          { clientX: WEEK_WIDTH } as MouseEvent,
          JAN_8,
        );
      });
      act(() => {
    ...
    it("should decrease duration when dragging left", () => {
      ...
      act(() => {
        result.current.startResize(task, "right", 0);
      });
      act(() => {
        result.current.handleResize(
          { clientX: -2 * WEEK_WIDTH } as MouseEvent,
          JAN_8,
        );
      });
      act(() => {
    ...
    it("should enforce minimum duration of 1 when dragging too far left", () => {
      ...
      act(() => {
        ...
        result.current.handleResize(
          { clientX: -10 * WEEK_WIDTH } as MouseEvent,
          JAN_8,
        );
      });
      act(() => {
  ...
  describe("week view — left handle", () => {
    it("should move start date forward and shorten duration", () => {
      ...
      act(() => {
        result.current.startResize(task, "left", 0);
      });
      act(() => {
  ...
  describe("month view — right handle", () => {
    it("should increase duration when dragging right across a month boundary", () => {
      ...
      const initialEndPixel = calculatePixelOffset(
        "month",
        timelineStart,
        task.endDate,
      );
      ...
      act(() => {
        result.current.startResize(task, "right", initialEndPixel);
      });
      act(() => {
      ...
      act(() => {
        result.current.endResize(timelineStart);
      });
  ...
  describe("month view — left handle", () => {
    it("should return null (no callback) when proposed start is past the end date", () => {
      ...
      const initialStartPixel = calculatePixelOffset(
        "month",
        timelineStart,
        task.startDate,
      );
      ...
      act(() => {
        result.current.startResize(task, "left", initialStartPixel);
      });
      act(() => {
        // Drag far right: proposed start moves past the end date
        result.current.handleResize(
          { clientX: initialStartPixel + 600 } as MouseEvent,
          timelineStart,
        );
      });
      act(() => {
    ...
    it("should shorten duration when dragging start forward", () => {
      const onResizeComplete = vi.fn();
      // 6-week task starting Jan 1; end date is Feb 11 (Sun)
      ...
      act(() => {
        // computeMonthLeftResize snaps the proposed start to the month boundary, so we
        // must drag far enough to cross into February (Jan = 31 * BASE_DAY_WIDTH ≈ 443px).
        // Dragging to pixel 500 lands in February → proposedStartDate = Feb 1,
        // which is genuinely later than Jan 1, shrinking the duration.
        result.current.handleResize(
          { clientX: 500 } as MouseEvent,
          timelineStart,
        );
      });
      act(() => {
